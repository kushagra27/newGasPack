...
class EditChallan extends React.Component{
                ...
                balance.map( balanceItem =>{
                    obj.dispatchItem.cylinders.map(cylItem =>{
                        if(cylItem.gas === balanceItem.gas){
                            ...
                            balanceItem.quantity -= parseInt((_.find(this.state.selectedChallan.cylinders, {gas: balanceItem.gas})).quantity)
    ...
    updateStock = (obj)=>{
        ...
        var challanRef = db.collection('stocks').doc(this.state.soldFrom).collection(this.handleDate(this.state.dateSold)).doc('filled')
        ...
        return db.runTransaction(async (transaction) => {
            return transaction.get(challanRef).then((doc) => {
                ...
                if (!doc.exists) {
                    ...
                    var newChallans = doc.data().challans.filter((chl)=>{ return chl.challanNumber!== this.state.selectedChallan.challanNumber})
                          ...
                          <Row>
                            <Col lg={6}>
                              <Form.Group controlId="formBasicSO">
                                ...
                                <Form.Text className="text-muted">Challan Number</Form.Text>
                              </Form.Group>
