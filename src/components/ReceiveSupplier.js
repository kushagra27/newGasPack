...
class ReceiveSupplier extends React.Component {
  ...
  createGas = () => {
    return this.props.gas.map((item) => {
      return (
        <td>
          <input
            style={{
      ...
      return db.runTransaction(async (transaction) => {
        return transaction
          ...
          .then((doc) => {
            ...
            if (!doc.exists) {
              challanRef
                .set({ er: this.state.locationWiseEntry[location] })
                .then(() => {});
              console.log("New Document created");
            } else if (!doc.data().receive) {
              challanRef
                .update({ receive: this.state.locationWiseEntry[location] })
                .then(() => {});
            } else {
              console.log("in else");
              ...
              var newER = doc
                .data()
                .er.concat(this.state.locationWiseEntry[location]);
              console.log(newER);
          ...
          <Row>
            ...
            <Col lg={10} id="page-content-wrapper">
              <div className="d-flex justify-content-center mt-3">
                <h2>Receive from the Supplier</h2>
              </div>
                        ...
                        <tr>
                          ...
                          <td>
                            <Button
                              ...
                              onClick={() => {
                                this.handleRemove(item.erNumber);
                              }}
                            >
                              Remove
                            </Button>
